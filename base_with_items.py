...
while True:
    ...
    for index, card in enumerate(cards_in_hand):
        if card.cost <= me.mana:
            creatures_to_summon.append(card)
            if 'C' in card.abilities:
                creatures.append(card)
            me.mana -= card.cost

    creatures_summoned = []
    for card in creatures_to_summon:
        result_str += "SUMMON {};".format(card.cid)
        creatures_summoned.append(card)

    for card in creatures_to_summon:
        if card.ctype == 1 and len(creatures_summoned) != 0:
            result_str += 'USE {} {};'.format(card.cid, creatures_summoned[0].cid)
        elif card.ctype == 2 and len(opponent_creatures) != 0:
            result_str += 'USE {} {};'.format(card.cid, opponent_creatures[0].cid)
        elif card.ctype == 3:
            result_str += 'USE {} -1;'.format(card.cid)
